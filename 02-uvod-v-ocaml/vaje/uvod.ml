...
let starting_element list = List.hd list
...
let rec sum_int_pairs l =
  let sum_pair (a, b) = a + b in
  begin match l with
  | [] -> []
  | h::t -> sum_pair h :: sum_int_pairs t
  end
...
let rec insert x k list =
  if k <= 0 then x::list else 
    if k > List.length list then insert x (List.length list) list  
      else List.hd list :: insert x (k-1) (List.tl list)
...
if k <= 0 then ([], list) else
  if k >= List.length list then (list, []) else
    let (x, y) = divide (k - 1) (List.tl list) in
    (x @ [List.hd list], y)
...
let is_palindrome list =
  ...
  List.equal (fun x y -> x = y) list rev
...
| [], _ -> []
| _, [] -> []
| _,_ -> (max (List.hd list1) (List.hd list2))::max_on_components (List.tl list1) (List.tl list2)
(*----------------------------------------------------------------------------*]
...
let rec largest list =
  ...
  | 2 -> max (List.hd list) (List.hd (List.tl list))
  | _ -> max (List.hd list) (largest (List.tl list))


let second_largest list =
let m = (largest list) in
largest (remove m list)